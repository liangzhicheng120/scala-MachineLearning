...
/**
 * 用户数据文件 users.dat 用户ID::性别::年龄::职业编号::邮编
 * 电影数据文件 movies.dat 电影ID::电影名称::电影种类
 * 评分数据文件 ratings.dat 用户ID::电影ID::评分::时间
 * 测试数据 personRating 用户ID::电影ID::评分::时间
 */
object MovieLensALS {
  ...
  def main(args: Array[String]) {
    ...
    val myRatings = loadRatings("D:\\data\\my.csv")
    val myRatingsRDD = sc.parallelize(myRatings, 1)
    //样本数据目录
    val movielensHomeDir = "D:\\data"
    //装载样本评分数据，其中最后一列Timestamp取除10的余数作为key，Rating为值，即(Int，Rating)
    val ratings = sc.textFile(movielensHomeDir + "\\ratings.csv").map {
      line =>
    ...
    //装载电影目录对照表(电影ID->电影标题)
    val movies = sc.textFile(movielensHomeDir + "\\movies.csv").map {
      line =>
        val fields = line.split(",")
